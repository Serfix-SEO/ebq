...
class ActionQueueService
{
    ...
    public function groupedActions(string $websiteId, ?string $country = null, bool $includeCrawlIssues = true): array
    {
        ...
        $items = [
            $this->summary('indexing_fails', __('Indexing failures with traffic'),
                __('Pages Google isn’t indexing that still earn impressions.'),
                self::SEV_CRITICAL, count($indexing), 0.0, __('Fix')),
            $this->summary('cannibalization', __('Keyword cannibalization'),
                __('Multiple pages competing for the same query and splitting clicks.'),
                self::SEV_HIGH, count($cannibalization), 0.0, __('View')),
            $this->summary('content_decay', __('Decaying pages'),
                __('Pages losing clicks fast that still earn impressions — recoverable.'),
                self::SEV_HIGH, count($decay), 0.0, __('View')),
            $this->summary('rank_drops', __('Ranking drops'),
                __('Tracked keywords that fell 5+ positions since the last check.'),
                self::SEV_HIGH, count($rankDrops), 0.0, __('View')),
            $this->summary('audit_performance', __('Slow pages costing traffic'),
                __('Audited pages scoring under 70 that still pull real traffic.'),
                self::SEV_HIGH, count($auditPerf), 0.0, __('View')),
            $this->summary('striking_distance', __('Striking-distance keywords'),
                __('Ranking positions 5–20 with traffic — small pushes reach page one.'),
                self::SEV_GROWTH, count($striking), 0.0, __('View')),
            $this->summary('quick_wins', __('Quick-win keywords'),
                __('High-volume, low-competition terms you don’t yet rank for.'),
