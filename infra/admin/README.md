...
|---|---|---|
| Clients | `app/Http/Controllers/Admin/ClientController.php:17` | Paginated user list w/ per-user website count, last activity, KE/SERP spend MTD (correlated sub-selects). Create/update, bulk enable/disable, **force-apply (comp) a plan**, admin recrawl. The list renders **twice** — cards below `md`, table from `md` up (`resources/views/admin/clients/index.blade.php`) — sharing `partials/{identity,status-badges,publishing-status,edit-panel}.blade.php` so the layouts can't drift. Each row names the client's **website domains** (two inline, the rest folded into a `+N` whose tooltip lists them all) — admins recognise accounts by domain, not email; they come from the already eager-loaded `websites` relation, so no per-row query. The **Publishing** column rolls a client's publishing connection up across every website they own (No website / Not connected / Connected / Connection error / Pending) from three `content_integrations` sub-selects (`integrations_connected|_error|_total`); platform names come from the eager-loaded `websites.contentIntegrations`, so the column costs no per-row query. It also pays for its width: `Joined` is `2xl:table-cell`, the impersonate label collapses to its icon below `2xl`, and the row shows a 6-char ULID tail (full id in the tooltip). User ids are ULIDs: every id entering an Alpine expression goes through `@js()` (an `(int)` cast made `isSelected(01m0…)` a JS syntax error and bulk select silently did nothing until 2026-08-14). |
| Client detail | `app/Http/Controllers/Admin/ClientController.php:161` (`show`) + `app/Services/Admin/ClientProfileService.php` | `/admin/clients/{user}` — one screen per account: identity/status badges, KPI strip, account facts, billing + entitlements (Cashier subs, card, content trial/comp/add-ons), every website with its Content Autopilot plan, publishing integrations (see below) and 28-day GSC/GA numbers, production stats (topics by status, avg SEO score, words, article feedback), tracked-keyword position spread, API spend MTD/lifetime + 6-month trend, support tickets, lifecycle emails, activity feed, and the shared edit panel (`return_to=show` bounces the save back here). All aggregation lives in the service; `search_console_data` (~2M rows) is the only big table and is cached 30min per client. Month bucketing switches DATE_FORMAT/strftime by driver (sqlite runs the tests). **Publishing integrations** render per website: platform, status, verification time, delivery counters (delivered/failed/queued + last delivery, from `content_publications` grouped by `integration_id`), last error, and the destination address from `ContentIntegration::targetSummary()` — an **opt-in allow-list** (site URL, webhook endpoint, store/collection/project/dataset/blog ids, post status). `credentials` is an encrypted cast holding tokens, API keys, app passwords and webhook secrets: never render it, and never turn `targetSummary()` into a blanket dump when a driver gains new credential keys. |
