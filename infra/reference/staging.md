...
| Web | `https://staging.serfix.io` → box A reverse-proxy vhost (`/etc/apache2/sites-available/staging.serfix.io.conf`) → `http://10.0.0.4`. Basic auth (`/etc/apache2/.htpasswd-staging`, user `serfix`). `X-Robots-Tag: noindex` at BOTH hops |
...
- TLS: pending DNS. Operator must add Cloudflare DNS record
  `staging.serfix.io` → A `138.199.217.239` (same as serfix.io, box A), then run
  `certbot --apache -d staging.serfix.io` on box A. Until then it's reachable
  via `curl -H "Host: staging.serfix.io" http://138.199.217.239/` or hosts-file.
