...
| `REDIS_DB`/`REDIS_CACHE_DB`/`REDIS_PREFIX` | `13`/`14`/`serfix-testing-` | Tests can never touch the prod Redis keyspace (2026-07-06). **And they're flushed once per phpunit process** by `TestCase::flushDedicatedTestRedisOnce()` (2026-07-23): unlike sqlite `:memory:`, dbs 13/14 persist across runs, so fixed-window counters leak — `MozSpendMeter`'s monthly 40-row cap filled purely from accumulated test runs and two `ContentCompetitorsTest` Moz tests failed with silent stale-nulls. The flush hard-aborts unless the resolved dbs are exactly 13/14 (cached-config protection, same as the DB guard). |
| `IDEOGRAM_MODEL` | `""` | Machine `.env` moved to `v4` (2026-07) and leaked into `IdeogramModelTest`'s "v3 default" assertion — non-billable flavor of the same anything-not-overridden-leaks rule below. |
