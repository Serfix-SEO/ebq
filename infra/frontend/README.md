...
| Marketing | `landing`, `features`, `pricing`, `content-landing`, `content-pricing`, `contact`, `guide`, `wordpress-plugin`, `website-revamp`, `legal/` | `x-marketing.page` | static `Route::view` pages |
...
### Two products, two pricing pages (2026-07-29)

`/pricing` (`pricing.blade.php`) prices the **SEO platform only**; Content AI Autopilot
has its own page at `/content-autopilot/pricing` (`content-pricing.blade.php`, route
`content.pricing`).

They used to share one grid behind an Alpine `product` pill toggle. First-time visitors
never noticed the toggle, so half the catalogue was invisible and the two products' numbers
read as one confusing price list. The toggle is gone: `/pricing` now ends with a full-width
"Looking for Content AI Autopilot?" band, and the header nav's *Pricing* item is a dropdown
naming both destinations (mobile nav + footer list both explicitly).

- **Every content price comes from `ContentAutopilotConfig`** — never hardcode it. The
  in-app Get started screen, the content landing page and this page all read the same
  source, so they cannot drift.
- **Product visuals** live in `public/images/content/*.webp` (calendar, article-score,
  tracker, rank-chart). They are REAL product screens, generated by
  `tests/Feature/Content/MarketingShotsTest.php` — it renders the actual Livewire
  components against a throwaway sqlite DB seeded with a fictional demo brand
  (northlanecoffee.com) and dumps the HTML for a headless screenshot. Regenerate with:
  `MARKETING_SHOTS=/tmp/shots php artisan test tests/Feature/Content/MarketingShotsTest.php`
  (the test skips unless that env var is set). Never screenshot a real client account for
  marketing — that is the whole reason this harness exists.
...
Tests: `tests/Feature/PricingPagesTest.php`.
