...
surfaces sit on one **provider-agnostic LLM client** (`LlmClient`, today only
Mistral):
...
| [llm.md](./llm.md) | `LlmClient` contract, `MistralClient`, model selection (`AiModelConfig`), function-calling loop, token/credit accounting, usage caps, non-secret config + env. |
...
Every AI feature flows through `App\Services\Llm\LlmClient`, bound to
`MistralClient` (`app/Providers/AppServiceProvider.php:41`), default model resolved
by `AiModelConfig::currentModel()` (admin Setting → `services.mistral.model` →
`mistral-small-latest`). Studio tools are PHP classes under `app/AiTools/Tools/*`,
...
4. **Every LLM call is billable.** `MistralClient` meters tokens through
...
- LLM — `app/Services/Llm/{LlmClient,MistralClient}.php`, `app/Support/AiModelConfig.php`,
