    ...
    <section class="relative">
        ...
        <div class="mx-auto max-w-4xl px-6 pb-16 pt-16 text-center lg:px-8 lg:pb-24 lg:pt-24">
            ...
            <div class="relative mx-auto mt-10 max-w-3xl">
                ...
                <form id="ga-form" class="text-start" data-action="{{ route('guest-audit.store') }}" novalidate>
                ...
                <div id="ga-success" class="hidden rounded-2xl border border-emerald-200 bg-white p-8 text-center shadow-[0_30px_70px_-28px_rgba(15,23,42,0.30)]">
                    ...
                    <a href="{{ route('register') }}" class="mt-6 inline-flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-800">{{ __('Create a free account for unlimited audits →') }}</a>
                </div>
            </div>
        </div>
    </section>
    ...
    <div id="ga-email-modal" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
        ...
        <div role="dialog" aria-modal="true" aria-labelledby="ga-email-title" class="relative w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-slate-900/5">
            <div class="px-7 pt-7">
                ...
                <p id="ga-email-modal-msg" class="mt-2 text-sm leading-6 text-slate-600">{{ __('This one’s on us — tell us where to send your report and we’ll deliver it in about a minute.') }}</p>
            </div>
            <form id="ga-email-form" class="px-7 pb-7 pt-5" novalidate>
                <div class="space-y-3">
                    <div>
                        ...
                        <input id="ga-name" name="name" type="text" autocomplete="name" maxlength="120" required placeholder="{{ __('Jane Doe') }}"
                    ...
                    <div>
                        ...
                        <input id="ga-email" name="email" type="email" autocomplete="email" inputmode="email" required placeholder="{{ __('you@company.com') }}"
                            class="block w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 transition focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500">
                    </div>
                </div>
                @if (\App\Support\Recaptcha::isEnabled())
                ...
                <div class="mt-5 flex flex-col gap-2 sm:flex-row-reverse">
                    ...
                    <button type="button" id="ga-email-cancel" class="rounded-xl px-5 py-3 text-sm font-semibold text-slate-600 transition hover:bg-slate-100">{{ __('Cancel') }}</button>
                </div>
            </form>
        </div>
    </div>
