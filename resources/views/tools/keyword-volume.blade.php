...
>
    <x-slot:schema>
        @php
            $toolSchema = [
                ...
                '@type' => 'WebApplication',
                'name' => 'Serfix Keyword Volume Checker',
    ...
    <section class="relative">
        ...
        <div class="mx-auto max-w-3xl px-6 pb-16 pt-16 text-center lg:px-8 lg:pb-24 lg:pt-24">
            ...
            <p class="mx-auto mt-5 max-w-xl text-balance text-[17px] leading-8 text-slate-600">
                {{ __('See the monthly Google search volume, cost-per-click and competition for any keyword — in seconds.') }}
            </p>
                    ...
                    <div class="flex flex-col rounded-[20px] bg-white p-2 shadow-[0_30px_70px_-28px_rgba(15,23,42,0.30)] ring-1 ring-slate-200/80 transition focus-within:ring-2 focus-within:ring-orange-500/70 sm:flex-row sm:items-center sm:divide-x sm:divide-slate-200/70 divide-y divide-slate-100 sm:divide-y-0">
                        <div class="flex min-w-0 flex-1 items-center gap-3 px-3 py-2.5">
                            ...
                            <div class="min-w-0 flex-1">
                                <label for="kv-keyword" class="block text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-400">{{ __('Keyword') }}</label>
                                <input id="kv-keyword" name="keyword" type="text" autofocus required maxlength="200" placeholder="{{ __('best seo tools') }}"
                        ...
                        <div class="flex items-center gap-3 px-3 py-2.5 sm:w-44">
                            ...
                            <div class="min-w-0 flex-1">
                                <label for="kv-country" class="block text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-400">{{ __('Country') }}</label>
                                <select id="kv-country" name="country" class="-ms-0.5 w-full border-0 bg-transparent p-0 text-[15px] font-medium text-slate-900 focus:outline-none focus:ring-0">
                        ...
                        <div class="pt-2 sm:ps-2 sm:pt-0">
                            <button type="submit" id="kv-submit"
                                ...
                                <span id="kv-label">{{ __('Check volume') }}</span>
    ...
    <section class="border-t border-slate-200 bg-slate-50">
        <div class="mx-auto max-w-5xl px-6 py-16 lg:px-8">
            <h2 class="text-center text-2xl font-semibold tracking-tight text-slate-900">{{ __('What’s in your free volume check') }}</h2>
            <div class="mt-10 grid gap-6 sm:grid-cols-3">
                @foreach ([
                    [__('Monthly search volume'), __('The average monthly Google searches for your keyword, straight from Keyword Planner data.')],
                    [__('CPC & competition'), __('Top-of-page cost-per-click and advertiser competition, so you can gauge commercial intent.')],
                    [__('12-month trend'), __('How interest has moved over the past year — spot seasonality before you commit content.')],
                ] as $f)
    ...
    <script>
        (function () {
            ...
            function setLoading(on) { btn.disabled = on; form.setAttribute('aria-busy', on ? 'true' : 'false'); label.textContent = on ? @json(__('Checking…')) : @json(__('Check volume')); spinner.classList.toggle('hidden', !on); arrow.classList.toggle('hidden', on); }
