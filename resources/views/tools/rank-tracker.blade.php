...
>
    ...
    <section class="relative">
        ...
        <div class="mx-auto max-w-3xl px-6 pb-16 pt-16 text-center lg:px-8 lg:pb-24 lg:pt-24">
            ...
            <p class="mx-auto mt-5 max-w-xl text-balance text-[17px] leading-8 text-slate-600">
                {{ __('See exactly where your site ranks on Google for any keyword — and the top results you’re up against — in seconds.') }}
            </p>
            ...
            <div class="relative mx-auto mt-10 max-w-2xl">
                ...
                <form id="rk-form" class="text-start" data-action="{{ route('guest-rank.store') }}" novalidate>
                    <div class="flex flex-col rounded-[20px] bg-white p-2 shadow-[0_30px_70px_-28px_rgba(15,23,42,0.30)] ring-1 ring-slate-200/80 transition focus-within:ring-2 focus-within:ring-orange-500/70 sm:flex-row sm:items-center sm:divide-x sm:divide-slate-200/70 divide-y divide-slate-100 sm:divide-y-0">
                        <div class="flex min-w-0 flex-1 items-center gap-3 px-3 py-2.5">
                            ...
                            <div class="min-w-0 flex-1">
                                <label for="rk-domain" class="block text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-400">{{ __('Your domain') }}</label>
                                <input id="rk-domain" name="domain" type="text" inputmode="url" autocomplete="url" autofocus required placeholder="{{ __('yourwebsite.com') }}"
                        ...
                        <div class="flex items-center gap-3 px-3 py-2.5 sm:w-52">
                            ...
                            <div class="min-w-0 flex-1">
                                <label for="rk-keyword" class="block text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-400">{{ __('Keyword') }}</label>
                                <input id="rk-keyword" name="keyword" type="text" required maxlength="200" placeholder="{{ __('best seo tools') }}"
                                    class="w-full border-0 bg-transparent p-0 text-[15px] font-medium text-slate-900 placeholder:font-normal placeholder:text-slate-400 focus:outline-none focus:ring-0">
                            </div>
                        </div>
                        <div class="flex items-center gap-3 px-3 py-2.5 sm:w-44">
                            ...
                            <div class="min-w-0 flex-1">
                                <label for="rk-country" class="block text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-400">{{ __('Country') }}</label>
                                <select id="rk-country" name="country" class="-ms-0.5 w-full border-0 bg-transparent p-0 text-[15px] font-medium text-slate-900 focus:outline-none focus:ring-0">
                    ...
                    <p class="mt-5 flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-xs text-slate-500">
                        ...
                        <span class="text-slate-300">·</span><span>{{ __('No signup for your first check') }}</span>
                ...
                <div id="rk-success" class="hidden rounded-2xl border border-emerald-200 bg-white p-8 text-center shadow-[0_30px_70px_-28px_rgba(15,23,42,0.30)]">
                    ...
                    <p id="rk-success-msg" class="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-600">{{ __('We’ve emailed your rank report. It lands in a minute.') }}</p>
                    <a href="{{ route('register') }}" class="mt-6 inline-flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-800">{{ __('Create a free account for unlimited checks →') }}</a>
                </div>
            </div>
        </div>
    </section>
    ...
    <div id="rk-email-modal" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
        ...
        <div role="dialog" aria-modal="true" aria-labelledby="rk-email-title" class="relative w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-slate-900/5">
            <div class="px-7 pt-7">
                ...
                <h2 id="rk-email-title" class="mt-4 text-xl font-semibold tracking-tight text-slate-900">{{ __('We’ll email you this report') }}</h2>
                <p id="rk-email-modal-msg" class="mt-2 text-sm leading-6 text-slate-600">{{ __('This one’s on us — tell us where to send your rank report and we’ll deliver it in about a minute.') }}</p>
            </div>
            <form id="rk-email-form" class="px-7 pb-7 pt-5" novalidate>
                <div class="space-y-3">
                    <div>
                        <label for="rk-name" class="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-500">{{ __('Your name') }}</label>
                        <input id="rk-name" name="name" type="text" autocomplete="name" maxlength="120" required placeholder="{{ __('Jane Doe') }}"
                    ...
                    <div>
                        <label for="rk-email" class="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-500">{{ __('Email address') }}</label>
                        <input id="rk-email" name="email" type="email" autocomplete="email" inputmode="email" required placeholder="{{ __('you@company.com') }}"
                            class="block w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 transition focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500">
                    </div>
                </div>
                @if (\App\Support\Recaptcha::isEnabled())
                ...
                <div class="mt-5 flex flex-col gap-2 sm:flex-row-reverse">
                    <button type="submit" id="rk-email-submit"
                        ...
                        <span id="rk-email-label">{{ __('Email me my report') }}</span>
                    </button>
                    <button type="button" id="rk-email-cancel" class="rounded-xl px-5 py-3 text-sm font-semibold text-slate-600 transition hover:bg-slate-100">{{ __('Cancel') }}</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <section class="border-t border-slate-200 bg-slate-50">
        <div class="mx-auto max-w-5xl px-6 py-16 lg:px-8">
            <h2 class="text-center text-2xl font-semibold tracking-tight text-slate-900">{{ __('What’s in your free rank check') }}</h2>
            <div class="mt-10 grid gap-6 sm:grid-cols-3">
                @foreach ([
                    [__('Your exact position'), __('The real organic position of your domain on Google for the keyword — scanned across the first 100 results.')],
                    [__('Who you’re competing with'), __('The top results ranking for that keyword, with your own listing highlighted so you see the gap.')],
                    [__('Country targeting'), __('Run the check from the country that matters — rankings are localized, and so is this tool.')],
                ] as $f)
    ...
    <script>
        (function () {
            ...
            function setLoading(on) { btn.disabled = on; form.setAttribute('aria-busy', on ? 'true' : 'false'); label.textContent = on ? @json(__('Checking…')) : @json(__('Check rank')); spinner.classList.toggle('hidden', !on); arrow.classList.toggle('hidden', on); }
            ...
            function run() {
                ...
                if (!domain) { showError(@json(__('Please enter your domain.'))); return; }
                if (!keyword) { showError(@json(__('Please enter a keyword.'))); return; }
