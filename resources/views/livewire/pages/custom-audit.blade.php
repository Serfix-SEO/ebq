...
<div class="mx-auto max-w-4xl space-y-6" @if ($hasPending) wire:poll.3s @endif>
    ...
    <header class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <div class="mt-3 flex flex-wrap items-start justify-between gap-3">
                <div class="min-w-0">
                    <div class="flex items-center gap-2">
                        <h1 class="text-xl font-bold tracking-tight text-slate-900 dark:text-slate-100 sm:text-2xl">{{ __('Custom page audit') }}</h1>
                        <x-guide-link anchor="custom-audit" />
                    </div>
                    <p class="mt-1 max-w-2xl text-sm leading-relaxed text-slate-500 dark:text-slate-400">
                        {{ __('Run an audit for any URL on the selected site and set the') }} <span class="font-medium text-slate-700 dark:text-slate-300">{{ __('SERP benchmark keyword') }}</span> {{ __('yourself. Search Console’s primary query is not used for the SERP snapshot on this flow.') }}
                    </p>
                </div>
                @if ($website)
                    <span class="inline-flex shrink-0 items-center rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-semibold text-slate-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200" title="{{ __('Current website') }}">
                        {{ $website->domain }}
                    </span>
                @endif
            </div>
        ...
        <p class="px-5 py-3 text-[11px] leading-relaxed text-slate-500 dark:text-slate-400">
            {{ __('Audits fetch the live page, run checks, and may take up to a minute. You will confirm the Google SERP country before the run starts when we can infer locale from the page.') }}
        </p>
    </header>
        ...
        <div class="overflow-hidden rounded-2xl border border-orange-300 bg-orange-50 shadow-sm dark:border-orange-500/40 dark:bg-orange-500/10">
            <div class="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex gap-3">
                    ...
                    <div class="min-w-0">
                        <p class="text-sm font-bold text-orange-900 dark:text-orange-100">{{ __('You’re running audits in limited mode') }}</p>
                        <p class="mt-1 max-w-xl text-xs leading-relaxed text-orange-800/90 dark:text-orange-200/80">
                            {{ __('No Google Analytics or Search Console is connected for') }} <span class="font-semibold">{{ $website->domain ?: __('this website') }}</span>{{ __(', so audits can’t show real traffic, keyword rankings, impressions or indexing status. Connect a source to unlock the full audit.') }}
                        </p>
                    </div>
                </div>
                <button
    ...
    @if ($websiteId === 0)
        <div class="rounded-2xl border border-amber-200 bg-amber-50/90 px-5 py-4 shadow-sm dark:border-amber-900/50 dark:bg-amber-500/10">
            <div class="flex gap-3">
                ...
                <div>
                    <p class="text-sm font-semibold text-amber-900 dark:text-amber-200">{{ __('Select a website first') }}</p>
                    <p class="mt-1 text-xs leading-relaxed text-amber-800/90 dark:text-amber-200/80">{{ __('Choose a site from the header, then return here to run a custom audit.') }}</p>
                    <a href="{{ route('websites.index') }}" wire:navigate class="mt-3 inline-flex text-xs font-semibold text-amber-900 underline decoration-amber-700/40 underline-offset-2 hover:decoration-amber-900 dark:text-amber-100 dark:hover:text-white">{{ __('Manage websites') }}</a>
                </div>
            </div>
        </div>
    @else
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div class="border-b border-slate-100 px-5 py-3 dark:border-slate-800">
                <h2 class="text-sm font-bold text-slate-900 dark:text-slate-100">{{ __('Run audit') }}</h2>
                <p class="mt-0.5 text-[11px] text-slate-500 dark:text-slate-400">{{ __('URLs must stay on the current site’s domain.') }} <code class="rounded bg-slate-100 px-1 py-px font-mono text-[10px] text-slate-700 dark:bg-slate-800 dark:text-slate-300">https://</code> {{ __('is added when omitted.') }}</p>
            </div>

            <form wire:submit="queueAudit" class="space-y-5 p-5" novalidate>
                <div class="space-y-5">
                    <div>
                        <label for="custom-audit-url" class="block text-xs font-semibold uppercase tracking-wide text-slate-700 dark:text-slate-300">{{ __('Page URL') }}</label>
                        <input
                            id="custom-audit-url"
                            type="url"
                            inputmode="url"
                    ...
                    <div>
                        <label for="custom-audit-keyword" class="block text-xs font-semibold uppercase tracking-wide text-slate-700 dark:text-slate-300">{{ __('Target keyword (SERP)') }}</label>
                        <p class="mt-0.5 text-[11px] text-slate-500 dark:text-slate-400">{{ __('Drives SERP organic results, in-sample rank checks, and competitor readability averages.') }}</p>
                        <input
                            id="custom-audit-keyword"
                            type="text"
                            autocomplete="off"
                            wire:model.blur="targetKeyword"
                            placeholder="{{ __('e.g. best project management software') }}"
                            @class([
                                'mt-1.5 block w-full rounded-lg border bg-white px-3 py-2.5 text-sm text-slate-900 shadow-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-slate-950 dark:text-slate-100 dark:placeholder:text-slate-500',
                ...
                @if ($awaitingSerpCountryChoice)
                    <div class="rounded-xl border border-orange-200 bg-orange-50/60 p-4 dark:border-orange-900/50 dark:bg-orange-500/10" role="region" aria-labelledby="custom-audit-serp-heading">
                        <div class="flex flex-wrap items-center justify-between gap-2">
                            <h3 id="custom-audit-serp-heading" class="text-sm font-bold text-slate-900 dark:text-slate-100">{{ __('Confirm SERP country') }}</h3>
                            <span class="rounded-full bg-white/80 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-orange-700 ring-1 ring-orange-200 dark:bg-slate-900/80 dark:text-orange-300 dark:ring-orange-800">{{ __('Step 2 of 2') }}</span>
                        </div>
                        ...
                        @else
                            <p class="mt-2 text-xs leading-relaxed text-slate-600 dark:text-slate-400">{{ __('Choose the regional Google index used for the organic snapshot. A best guess is pre-selected from your page.') }}</p>
                        @endif
                        <label for="custom-audit-serp-gl" class="mt-3 block text-[11px] font-semibold text-slate-700 dark:text-slate-300">{{ __('Country') }}</label>
                        <select
                ...
                <div class="flex flex-col gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800">
                    ...
                    <p wire:loading.remove wire:target="queueAudit" class="text-[11px] text-slate-500 dark:text-slate-400">{{ __('First submit checks the page and may ask for SERP region. Second submit queues the audit — it runs in the background.') }}</p>
                    <p wire:loading wire:target="queueAudit" class="text-[11px] font-medium text-orange-600 dark:text-orange-400">{{ __('Queuing…') }}</p>
                </div>
            </form>
        </div>
        ...
        <section class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900" aria-labelledby="custom-audit-history-heading">
            <div class="flex flex-col gap-1 border-b border-slate-100 px-5 py-4 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h2 id="custom-audit-history-heading" class="text-sm font-bold text-slate-900 dark:text-slate-100">{{ __('Recent audits') }}</h2>
                    <p class="mt-1 max-w-2xl text-[11px] leading-relaxed text-slate-500 dark:text-slate-400">
                        {{ __('Runs from this page, WordPress SEO Analysis, and page-level audits on Serfix. Opening the report shows the') }} <span class="font-medium text-slate-600 dark:text-slate-300">{{ __('latest saved audit') }}</span> {{ __('for that URL.') }}
                    </p>
                </div>
                @if ($recentAudits->isNotEmpty())
                    <span class="text-[11px] font-medium text-slate-400 dark:text-slate-500">{{ __('Last') }} {{ $recentAudits->count() }} {{ __('on this site') }}</span>
                @endif
            </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 text-start text-xs dark:divide-slate-700">
                        <thead class="bg-slate-50 text-[11px] font-semibold uppercase tracking-wide text-slate-500 dark:bg-slate-800/80 dark:text-slate-400">
                            <tr>
                                <th scope="col" class="whitespace-nowrap px-4 py-3">{{ __('When') }}</th>
                                <th scope="col" class="whitespace-nowrap px-4 py-3">{{ __('Source') }}</th>
                                <th scope="col" class="min-w-[10rem] px-4 py-3">{{ __('Page') }}</th>
                                <th scope="col" class="min-w-[7rem] px-4 py-3">{{ __('Market') }}</th>
                                <th scope="col" class="min-w-[8rem] px-4 py-3">{{ __('Keyword') }}</th>
                                <th scope="col" class="whitespace-nowrap px-4 py-3">{{ __('By') }}</th>
                                <th scope="col" class="whitespace-nowrap px-4 py-3">{{ __('Status') }}</th>
                                <th scope="col" class="whitespace-nowrap px-4 py-3 text-end">{{ __('Report') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                            @foreach ($recentAudits as $row)
                                <tr class="bg-white transition hover:bg-slate-50/80 dark:bg-slate-900 dark:hover:bg-slate-800/40">
                                    ...
                                    <td class="max-w-[10rem] truncate px-4 py-3 text-slate-600 dark:text-slate-300" title="{{ $row->serp_sample_gl ? __('SERP country: ').$row->serp_sample_gl : '' }}">
                                        @php
                                            ...
                                            if ($mkt === null && filled($row->serp_sample_gl)) {
                                                $mkt = __('SERP: ').\App\Support\Audit\SerpGlCatalog::labelFor($row->serp_sample_gl).' ('.$row->serp_sample_gl.')';
                                            }
                                        @endphp
                                        {{ $mkt ?? '—' }}
                                    </td>
                                    ...
                                    <td class="whitespace-nowrap px-4 py-3">
                                        @switch($row->status)
                                            ...
                                            @case(\App\Models\CustomPageAudit::STATUS_COMPLETED)
                                                <span class="inline-flex rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-semibold text-emerald-800 dark:bg-emerald-500/15 dark:text-emerald-300">{{ __('Done') }}</span>
                                                @break
