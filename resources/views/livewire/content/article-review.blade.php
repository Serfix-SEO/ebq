    ...
    @else
        ...
        <div class="grid gap-6 lg:grid-cols-3" @if($topic?->status === \App\Models\ContentTopic::STATUS_PUBLISHING) wire:poll.5s @endif>
            ...
            <div class="lg:col-span-2">
                ...
                @if (! empty($inlineImages) && count($inlineImages) > 0)
                    <div class="mb-4 rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900">
                        <p class="text-xs font-bold text-slate-700 dark:text-slate-200">{{ __('Images in this article') }}</p>
                        <div class="mt-2 space-y-2">
                            @foreach ($inlineImages as $img)
                                <div class="flex items-center gap-3" wire:key="inline-img-{{ $img->id }}" x-data="{ regenerating: false }">
                                    <img src="{{ $img->url() }}" alt="{{ $img->alt_text }}" class="h-12 w-20 shrink-0 rounded-lg object-cover" :class="regenerating ? 'opacity-40' : ''" />
                                    <span class="min-w-0 flex-1 truncate text-xs text-slate-500 dark:text-slate-400">{{ $img->alt_text ?: __('Article image') }}</span>
                                    @if (trim((string) $img->prompt) !== '')
                                        <button type="button"
                                            x-on:click="regenerating = true; $wire.regenerateImage('{{ $img->id }}').then(id => { if (! id) { regenerating = false; return; } const poll = setInterval(async () => { const r = await $wire.pollInlineImage(id); if (r && (r.url || r.failed)) { clearInterval(poll); regenerating = false; $wire.$refresh(); } }, 2500); setTimeout(() => { clearInterval(poll); regenerating = false; }, 90000); })"
                                            :disabled="regenerating"
                                            class="shrink-0 rounded-lg border border-slate-300 px-2.5 py-1 text-[11px] font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">
