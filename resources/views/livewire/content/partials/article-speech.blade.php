...
<div x-data="articleSpeech" x-cloak x-show="supported"
    ...
    {{-- Stop + speed + progress, only while there is something to control --}}
    <template x-if="speaking || paused">
        <div class="flex items-center gap-1.5">
            <button type="button" x-on:click="stop()" :aria-label="'{{ __('Stop reading') }}'"
                    class="inline-flex shrink-0 items-center justify-center rounded-lg border border-slate-300 p-2.5 text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">
            ...
            <select x-on:change="setRate($event.target.value)" :aria-label="'{{ __('Reading speed') }}'"
                    class="rounded-lg border border-slate-300 bg-white py-2.5 ps-2 pe-7 text-xs font-semibold text-slate-700 focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200">
