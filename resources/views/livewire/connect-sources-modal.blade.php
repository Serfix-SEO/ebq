        ...
        >
            <div class="flex items-center justify-between border-b border-slate-200 px-5 py-3.5 dark:border-slate-800">
                <div>
                    ...
                    <p class="mt-0.5 text-[11px] text-slate-500 dark:text-slate-400">{{ config('features.seo_platform_ui')
                        ? __('Attach a GA4 property and/or Search Console site to unlock the full report.')
                        : __('Search Console lets us submit your articles to Google. Analytics shows how many visitors each article brings.') }}</p>
                </div>
            ...
            <div class="px-5 py-4">
                ...
                <div wire:loading.flex wire:target="open" class="items-center justify-center gap-2 py-8 text-xs text-slate-500" role="status">
                    ...
                    {{ __('Loading your Google properties…') }}
                </div>
                ...
                <div wire:loading.remove wire:target="open">
                    ...
                    @elseif (! $loaded)
                        <p class="py-6 text-center text-xs text-slate-500">{{ __('Preparing…') }}</p>
                    @elseif (! $hasGoogle)
                        {{-- No Google account connected at all --}}
                        <p class="text-xs text-slate-600 dark:text-slate-300">{{ __('Connect a Google account to pull Analytics and Search Console data.') }}</p>
                        <a href="{{ route('google.redirect', ['return' => $googleReturn]) }}"
                    ...
                    @else
                        ...
                        <form wire:submit="saveSources" class="space-y-3">
                            <div>
                                ...
                                @if ($gaHint !== '')
                                    <p class="mb-1.5 rounded-md bg-amber-50 px-2.5 py-2 text-[11px] leading-relaxed text-amber-800 dark:bg-amber-900/30 dark:text-amber-200">{{ $gaHint }}</p>
                                @endif
                            ...
                            <a href="{{ route('google.redirect', ['return' => $googleReturn]) }}" class="block text-[11px] font-semibold text-orange-600 hover:text-orange-700 dark:text-orange-400">
                                {{ __('Analytics or Search Console under a different Google login? Connect that login →') }}
                            </a>

                            <div class="flex items-center justify-end gap-2 border-t border-slate-100 pt-3 dark:border-slate-800">
                                <button type="button" x-on:click="show = false" class="inline-flex h-8 items-center rounded-md px-3 text-xs font-medium text-slate-500 transition hover:text-slate-700 dark:hover:text-slate-300">{{ __('Cancel') }}</button>
                                <button type="submit" wire:loading.attr="disabled" wire:target="saveSources" class="inline-flex h-8 items-center rounded-md bg-orange-600 px-3 text-xs font-semibold text-white shadow-sm transition hover:bg-orange-700 disabled:opacity-60">
                                    <span wire:loading.remove wire:target="saveSources">{{ __('Save & sync') }}</span>
                                    <span wire:loading wire:target="saveSources">{{ __('Saving…') }}</span>
                                </button>
                            </div>
                        </form>
                    @endif
