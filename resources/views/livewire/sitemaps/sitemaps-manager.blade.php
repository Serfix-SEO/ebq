<div class="space-y-5" wire:init="autoSync">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-lg font-bold tracking-tight">{{ __('Sitemaps') }}</h1>
            <p class="mt-0.5 text-xs text-slate-500 dark:text-slate-400">{{ __('Add and track the XML sitemaps for this website') }}</p>
        </div>
    ...
    @unless ($hasGsc)
        <div class="flex items-start gap-2 rounded-md bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:bg-amber-900/30 dark:text-amber-200">
            ...
            <span>{{ __("No Google Search Console source is connected, so sitemaps can't be auto-fetched. You can still add and track sitemaps manually below.") }}</span>
        </div>
    @endunless
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
        <form wire:submit="addSitemap" class="flex flex-col gap-2 sm:flex-row sm:items-start">
            <div class="flex-1">
                <label class="mb-1 block text-[11px] font-medium text-slate-700 dark:text-slate-300">{{ __('Sitemap URL') }}</label>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-xs">
                    <thead class="border-b border-slate-200 bg-slate-50 text-[11px] uppercase tracking-wide text-slate-500 dark:border-slate-800 dark:bg-slate-800/50 dark:text-slate-400">
                        <tr>
                            <th class="px-4 py-2.5 font-medium">{{ __('Sitemap') }}</th>
                            <th class="px-4 py-2.5 font-medium">{{ __('Source') }}</th>
                            <th class="px-4 py-2.5 text-end font-medium">{{ __('URLs (indexed/submitted)') }}</th>
                            <th class="px-4 py-2.5 text-end font-medium">{{ __('Issues') }}</th>
                            <th class="px-4 py-2.5 font-medium">{{ __('Last downloaded') }}</th>
                            ...
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                                <td class="px-4 py-2.5">
                                    ...
                                    @if ($sitemap->is_sitemaps_index)
                                        <span class="ms-1.5 rounded bg-slate-100 px-1.5 py-0.5 text-[10px] font-medium text-slate-600 dark:bg-slate-800 dark:text-slate-300">{{ __('index') }}</span>
                                    @endif
                                    @if ($sitemap->is_pending)
                                        <span class="ms-1 rounded bg-amber-50 px-1.5 py-0.5 text-[10px] font-medium text-amber-700 dark:bg-amber-900/30 dark:text-amber-300">{{ __('pending') }}</span>
                                    @endif
                                </td>
                                <td class="px-4 py-2.5">
                                    @if ($sitemap->isFromGsc())
                                        <span class="inline-flex items-center rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-medium text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">{{ __('Search Console') }}</span>
                                    @else
                                        <span class="inline-flex items-center rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-medium text-slate-600 dark:bg-slate-800 dark:text-slate-300">{{ __('Added manually') }}</span>
                                    @endif
                                </td>
                                <td class="px-4 py-2.5 text-end tabular-nums text-slate-600 dark:text-slate-300">
                                ...
                                <td class="px-4 py-2.5 text-end tabular-nums">
                                    @if ($sitemap->errors > 0)
                                        <span class="text-red-600 dark:text-red-400">{{ $sitemap->errors }} {{ __('err') }}</span>
                                    @endif
                                    @if ($sitemap->warnings > 0)
                                        <span class="ms-1 text-amber-600 dark:text-amber-400">{{ $sitemap->warnings }} {{ __('warn') }}</span>
                                    @endif
                                ...
                                <td class="px-4 py-2.5 text-end">
                                    <button type="button" wire:click="removeSitemap('{{ $sitemap->id }}')"
                                        wire:confirm="{{ __('Remove this sitemap from Serfix? (It is not deleted from Google.)') }}"
                                        class="text-slate-400 transition hover:text-red-600 dark:hover:text-red-400" title="{{ __('Remove') }}">
