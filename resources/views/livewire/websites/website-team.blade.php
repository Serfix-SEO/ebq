<div wire:key="website-team-{{ $websiteId }}-{{ $useSessionWebsite ? 'session' : 'embed' }}">
    @if ($useSessionWebsite)
        <div class="mx-auto max-w-3xl space-y-2">
            <h1 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100">{{ __('Team & access') }}</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">
                {{ __('Invite collaborators and control which features they can use on this site. Access can be edited anytime.') }}
            </p>
        </div>
    @endif
    ...
    <div @class([
        ...
        @if ($emptyReason === 'select_website')
            <div class="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p class="mt-3 text-sm font-medium text-slate-600 dark:text-slate-300">{{ __('Select a website') }}</p>
                <p class="mt-1 max-w-sm text-xs text-slate-500 dark:text-slate-400">{{ __('Choose a site from the dropdown in the header, then return here to invite teammates.') }}</p>
            </div>
        @elseif ($emptyReason === 'no_access')
            <div class="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-100">
                {{ __('You do not have access to this website, or it no longer exists. Select another site in the header or open a site you own or have been invited to.') }}
            </div>
        @elseif ($website)
            @if ($useSessionWebsite)
                <div class="mb-6 flex flex-wrap items-end justify-between gap-4 border-b border-slate-100 pb-6 dark:border-slate-800">
                    ...
                    @if ($readonly)
                        <span class="inline-flex items-center rounded-md bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-600 dark:bg-slate-800 dark:text-slate-300">{{ __('View only') }}</span>
                    @endif
                </div>
            @else
                <h4 class="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('Team') }}</h4>
                <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ __('Invite collaborators by email and scope their access to specific features.') }}</p>
            @endif

            @if ($readonly)
                <p class="mb-6 text-sm text-slate-600 dark:text-slate-300">
                    {{ __('Only the site owner or an admin can send invites, remove members, or change access. You can still see who has access below.') }}
                </p>
            @endif
            ...
            @if (! $readonly)
                <form wire:submit="inviteMember" class="rounded-lg border border-slate-200 bg-slate-50/50 p-4 dark:border-slate-800 dark:bg-slate-800/30">
                    <div class="mb-3 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">{{ __('Invite a teammate') }}</div>
                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-[minmax(0,2fr)_minmax(0,1fr)_auto] sm:items-end">
                        <div>
                            <label for="invite-email-{{ $websiteId }}" class="mb-1 block text-[11px] font-medium text-slate-600 dark:text-slate-400">{{ __('Email address') }}</label>
                            <input id="invite-email-{{ $websiteId }}" wire:model="inviteEmail" type="email" placeholder="{{ __('colleague@example.com') }}" autocomplete="email"
                        ...
                        <div>
                            <label class="mb-1 block text-[11px] font-medium text-slate-600 dark:text-slate-400">{{ __('Role') }}</label>
                            <select wire:model.live="inviteRole" class="h-9 w-full rounded-md border border-slate-200 bg-white px-2.5 text-xs shadow-sm dark:border-slate-700 dark:bg-slate-800">
                                <option value="member">{{ __('Member (custom access)') }}</option>
                    ...
                    @if ($inviteRole === 'member')
                        <div class="mt-4 rounded-md border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-900">
                            <div class="mb-2 flex items-center justify-between">
                                <div class="text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">{{ __('Feature access') }}</div>
                                <div class="flex items-center gap-2 text-[10px]">
                                    <button type="button" wire:click="$set('invitePermissions', {{ json_encode(array_fill_keys(array_keys($features), true)) }})" class="text-orange-600 hover:underline">{{ __('Select all') }}</button>
                                    <span class="text-slate-300">·</span>
                                    <button type="button" wire:click="$set('invitePermissions', {{ json_encode(array_fill_keys(array_keys($features), false)) }})" class="text-slate-500 hover:underline">{{ __('Clear') }}</button>
                                </div>
                            </div>
                    ...
                    @else
                        <p class="mt-3 flex items-center gap-2 text-[11px] text-slate-500 dark:text-slate-400">
                            ...
                            {{ __('Admins have full access to every feature and can manage the team.') }}
                        </p>
                    @endif
                </form>
            @endif
            ...
            {{-- Members --}}
            <div class="mt-8">
                <h3 class="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('Members') }}</h3>
                <ul class="mt-3 space-y-2">
                    ...
                    <li class="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2.5 text-sm dark:bg-slate-800/50">
                        ...
                        <span class="rounded-md bg-orange-100 px-2 py-0.5 text-xs font-semibold uppercase tracking-wide text-orange-800 dark:bg-orange-500/20 dark:text-orange-200">{{ __('Owner') }}</span>
                    </li>
                            ...
                            <div class="flex flex-wrap items-center justify-between gap-3">
                                <div class="min-w-0">
                                    <div class="flex items-center gap-2">
                                        ...
                                        ])>{{ $role === 'admin' ? __('Admin') : __('Member') }}</span>
                                    </div>
                                    <span class="block text-xs text-slate-500 dark:text-slate-400">{{ $member->email }}</span>
                                    <div class="mt-1 text-[11px] text-slate-500 dark:text-slate-400">
                                        @if ($hasFullAccess)
                                            <span class="font-semibold text-emerald-700 dark:text-emerald-400">{{ __('Full access') }}</span>
                                        @else
                                            <span class="font-semibold">{{ __(':granted/:total features:', ['granted' => $grantedCount, 'total' => $featureCount]) }}</span>
                                            @foreach ($perms ?? [] as $p)
                                                <span class="me-1 inline-block rounded bg-slate-100 px-1.5 py-px text-[10px] text-slate-600 dark:bg-slate-800 dark:text-slate-300">{{ $features[$p]['label'] ?? $p }}</span>
                                            @endforeach
                                        @endif
                                    </div>
                                </div>
                                @if (! $readonly)
                                    <div class="flex shrink-0 items-center gap-1">
                                        ...
                                        <button type="button" wire:click="revokeMember('{{ $member->id }}')" wire:confirm="{{ __('Remove this member from the site?') }}"
            ...
            @if ($website->invitations->isNotEmpty())
                <div class="mt-8">
                    <h3 class="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('Pending invitations') }}</h3>
                    <ul class="mt-3 space-y-2">
                                ...
                                <div class="flex flex-wrap items-center justify-between gap-3">
                                    <div class="min-w-0">
                                        <div class="flex flex-wrap items-center gap-2">
                                            ...
                                            @if ($expired)
                                                <span class="rounded-full bg-red-50 px-2 py-px text-[10px] font-semibold uppercase text-red-600 dark:bg-red-500/10 dark:text-red-400">{{ __('Expired') }}</span>
                                            @else
                                                <span class="rounded-full bg-amber-50 px-2 py-px text-[10px] font-semibold uppercase text-amber-700 dark:bg-amber-500/10 dark:text-amber-400">{{ __('Pending') }}</span>
                                            @endif
                                            <span @class([
                                                'rounded-md px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide',
                                            ...
                                            ])>{{ $invRole === 'admin' ? __('Admin') : __('Member') }}</span>
                                        </div>
                                        <div class="mt-0.5 text-[11px] text-slate-500 dark:text-slate-400">
                                            @if ($sentAt){{ __('Sent') }} {{ $sentAt->diffForHumans() }}@endif
                                            @if ($inv->expires_at)
                                                · {{ $expired ? __('Expired') : __('Expires') }} {{ $inv->expires_at->diffForHumans() }}
                                            @endif
                                        </div>
                                        <div class="mt-1 text-[11px] text-slate-500 dark:text-slate-400">
                                            @if ($invHasFullAccess)
                                                <span class="font-semibold text-emerald-700 dark:text-emerald-400">{{ __('Full access') }}</span>
                                            @else
                                                <span class="font-semibold">{{ __(':granted/:total features:', ['granted' => $invGrantedCount, 'total' => count($features)]) }}</span>
                                    ...
                                    @if (! $readonly)
                                        <div class="flex shrink-0 flex-wrap items-center gap-1">
                                            ...
                                            <button type="button"
                                                ...
                                                <span wire:loading.remove wire:target="resendInvitation('{{ $inv->id }}')">{{ __('Resend') }}</span>
                                                <span wire:loading wire:target="resendInvitation('{{ $inv->id }}')">{{ __('Sending…') }}</span>
                                            </button>
                                            <button type="button" wire:click="cancelInvitation('{{ $inv->id }}')" wire:confirm="{{ __('Cancel this invitation?') }}"
