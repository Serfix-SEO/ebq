<div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
    <div class="flex items-start justify-between gap-4">
        <div class="min-w-0">
            <h2 class="text-sm font-semibold">{{ __('Report branding') }}</h2>
            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                {{ __('Your logo and company details on every report email and the attached PDF.') }}
            </p>
        </div>
    </div>
    ...
    @if (! $allowed)
        ...
        <div class="mt-4 rounded-lg border border-orange-200 bg-orange-50 p-4 dark:border-orange-800/60 dark:bg-orange-900/30">
            <p class="text-sm font-semibold text-orange-900 dark:text-orange-200">{{ __('Upgrade to unlock report whitelabel') }}</p>
            <p class="mt-1 text-xs text-orange-800 dark:text-orange-300">
                {{ __('Your current plan sends reports with Serfix branding. Upgrade to send branded reports with your logo, company details, and a PDF attachment your clients can download.') }}
            </p>
            <a href="{{ route('billing.show') }}" class="mt-3 inline-flex items-center rounded-md bg-orange-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm transition hover:bg-orange-500">
                {{ __('View plans') }}
            </a>
        </div>
    @else
        ...
        <form wire:submit="save" class="mt-4 space-y-4">
            <div>
                <label class="text-xs font-semibold text-slate-700 dark:text-slate-300">{{ __('Company name') }}</label>
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="text-xs font-semibold text-slate-700 dark:text-slate-300">{{ __('Logo') }}</label>
                    @if ($current_logo_url)
                        <div class="mt-1 flex items-center gap-3 rounded-md border border-slate-200 bg-slate-50 p-2 dark:border-slate-700 dark:bg-slate-800/60">
                            <img src="{{ $current_logo_url }}" alt="{{ __('logo') }}" class="h-10 max-w-[120px] object-contain">
                            <button type="button" wire:click="removeLogo" class="text-[11px] font-semibold text-red-600 hover:underline">{{ __('Remove') }}</button>
                        </div>
                    @endif
                    <input type="file" wire:model="logo" accept="image/png,image/jpeg,image/svg+xml"
                        class="mt-1 block w-full text-xs file:me-3 file:rounded file:border-0 file:bg-slate-100 file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-slate-700 dark:file:bg-slate-800 dark:file:text-slate-200">
                    @error('logo') <p class="mt-1 text-[11px] text-red-600">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="text-xs font-semibold text-slate-700 dark:text-slate-300">{{ __('Accent color') }}</label>
                    <div class="mt-1 flex items-center gap-2">
                        ...
                        <input type="text" wire:model="accent_color" maxlength="7"
                            class="flex-1 rounded-md border-slate-300 text-sm shadow-sm focus:border-orange-500 focus:ring-orange-500 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100">
                    </div>
            ...
            <div>
                <label class="text-xs font-semibold text-slate-700 dark:text-slate-300">{{ __('Footer text') }}</label>
                <textarea wire:model="footer_text" rows="2" maxlength="2000" placeholder="{{ __('Confidential — for the addressee only.') }}"
                    class="mt-1 block w-full rounded-md border-slate-300 text-sm shadow-sm focus:border-orange-500 focus:ring-orange-500 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100"></textarea>
            </div>

            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="text-xs font-semibold text-slate-700 dark:text-slate-300">{{ __('Contact email') }}</label>
                ...
                <div>
                    <label class="text-xs font-semibold text-slate-700 dark:text-slate-300">{{ __('Contact phone') }}</label>
            ...
            <div>
                <label class="text-xs font-semibold text-slate-700 dark:text-slate-300">{{ __('Address') }}</label>
                <textarea wire:model="contact_address" rows="2" maxlength="2000"
                    class="mt-1 block w-full rounded-md border-slate-300 text-sm shadow-sm focus:border-orange-500 focus:ring-orange-500 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100"></textarea>
            </div>

            <div>
                <label class="text-xs font-semibold text-slate-700 dark:text-slate-300">{{ __('Reply-to email') }}</label>
                <input type="email" wire:model="reply_to_email" maxlength="191" placeholder="hello@your-agency.com"
                    class="mt-1 block w-full rounded-md border-slate-300 text-sm shadow-sm focus:border-orange-500 focus:ring-orange-500 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100">
                <p class="mt-1 text-[10px] text-slate-500">{{ __('Replies to the report email go here instead of the sending mailbox.') }}</p>
