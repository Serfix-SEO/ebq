<div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
    <div class="flex items-start justify-between gap-4">
        <div class="min-w-0">
            ...
            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                Where report emails get sent from — your Gmail, Outlook, an SMTP server, or the Serfix default.
            </p>
        </div>
    </div>
    ...
    @if (! $allowed)
        <div class="mt-4 rounded-lg border border-orange-200 bg-orange-50 p-4 dark:border-orange-800/60 dark:bg-orange-900/30">
            <p class="text-sm font-semibold text-orange-900 dark:text-orange-200">Upgrade to send from your own mailbox</p>
            <p class="mt-1 text-xs text-orange-800 dark:text-orange-300">
                On your current plan, reports send from Serfix's default mailer. Upgrade to send reports from your own Gmail or Outlook account (better deliverability), or to plug in a custom SMTP server.
            </p>
            <a href="{{ route('billing.show') }}" class="mt-3 inline-flex items-center rounded-md bg-orange-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm transition hover:bg-orange-500">View plans</a>
        </div>
    @else
        @if ($saved)
        ...
        <form wire:submit="save" class="mt-4 space-y-4">
            ...
            <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
                ...
                @php $providers = [
                    ''        => ['Serfix default', 'Send from Serfix\'s default mailer. No branding sender, no setup.'],
            ...
            @if ($provider === 'gmail')
                <div class="rounded-md border border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-800/60">
                    ...
                    @else
                        ...
                        <a href="{{ route('google.mail.redirect') }}" class="mt-2 inline-block text-[11px] font-semibold text-orange-600 hover:underline">Reconnect with Gmail-send scope →</a>
                    @endif
                </div>
            @endif
            ...
            @if ($provider === 'outlook')
                <div class="rounded-md border border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-800/60">
                    ...
                    @else
                        ...
                        <a href="{{ route('microsoft.redirect') }}" class="mt-2 inline-block text-[11px] font-semibold text-orange-600 hover:underline">Connect another Outlook account →</a>
                    @endif
                </div>
            @endif
