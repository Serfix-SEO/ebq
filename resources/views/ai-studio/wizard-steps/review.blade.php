...
<div x-show="surface() === 'review'" class="space-y-4">
    <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
            <h3 class="text-base font-semibold text-slate-900 dark:text-slate-100">{{ __('Your article is ready') }}</h3>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ __('Preview below, switch to Edit for inline tweaks, then copy the HTML or download it.') }}</p>
        </div>
        <button type="button" @click="postGenView = 'wizard'; goToStep('summary')" class="rounded-md border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">{{ __('← Back to summary') }}</button>
    </div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-2">
        <div class="inline-flex rounded-lg border border-slate-200 bg-white p-0.5 text-xs font-semibold dark:border-slate-700 dark:bg-slate-900">
            <button type="button" @click="rv.mode = 'preview'" class="rounded-md px-3 py-1.5 transition" :class="rv.mode === 'preview' ? 'bg-orange-600 text-white' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'">{{ __('Preview') }}</button>
    ...
    <ul class="flex flex-wrap gap-4 rounded-lg bg-slate-50 px-4 py-2 text-sm dark:bg-slate-800/60" x-show="!gen.active">
        ...
        <li><strong x-text="reviewStats().images"></strong> <span class="text-slate-500 dark:text-slate-400">{{ __('images') }}</span></li>
        <li><strong x-text="reviewStats().links"></strong> <span class="text-slate-500 dark:text-slate-400">{{ __('links') }}</span></li>
        <li><strong x-text="'~' + reviewStats().minutes"></strong> <span class="text-slate-500 dark:text-slate-400">{{ __('min read') }}</span></li>
    </ul>
    ...
    <div x-show="rv.mode === 'edit'">
        <textarea x-model="rv.html" rows="22" spellcheck="false" placeholder="{{ __('Generated content will appear here.') }}"
        ...
        <p class="mt-1 text-[11px] text-slate-400">{{ __('Edits stay local to this preview. Use Copy / Download to take the HTML, or Regenerate to rebuild from the brief.') }}</p>
    </div>
    ...
    <template x-if="rv.saved"><div class="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-700 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-300">{{ __('Project marked complete. The generated HTML is saved on the project — copy or download it above to publish.') }}</div></template>
</div>
