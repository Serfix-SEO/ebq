...
<div x-show="surface() === 'brief'" class="space-y-5">
            ...
            <ul class="mt-4 space-y-3">
                <template x-for="(s, i) in b.brief.sections" :key="i">
                    <li class="rounded-lg border border-slate-200 p-3 dark:border-slate-800">
                        ...
                        <button type="button" @click="briefAddSub(i)" class="mt-2 ml-7 text-[11px] font-semibold text-orange-600 hover:underline dark:text-orange-400">+ Add subtopic</button>
                    </li>
                </template>
            </ul>
        ...
        <aside class="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-800/40">
            ...
            <div class="flex items-end gap-1.5">
                ...
                <button type="button" @click="briefSendChat()" :disabled="b.chatBusy || b.chatInput.trim().length === 0"
                    class="rounded-lg bg-orange-600 px-3 py-2 text-xs font-semibold text-white hover:bg-orange-700 disabled:opacity-50" x-text="b.chatBusy ? '…' : 'Send'"></button>
            </div>
        </aside>
    </div>
    ...
    <div class="flex items-center justify-between border-t border-slate-100 pt-4 dark:border-slate-800">
        ...
        <button type="button" @click="goToStep('strategy')" :disabled="b.brief.sections.length === 0" class="rounded-lg bg-orange-600 px-4 py-2 text-sm font-semibold text-white hover:bg-orange-700 disabled:bg-slate-300 disabled:text-slate-500 dark:disabled:bg-slate-700">Next: strategy →</button>
    </div>
</div>
