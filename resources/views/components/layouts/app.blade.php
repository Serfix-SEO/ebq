    ...
    <div class="min-h-screen md:flex">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : (document.documentElement.dir === 'rtl' ? 'translate-x-full' : '-translate-x-full')" class="fixed inset-y-0 start-0 z-40 flex w-64 flex-col border-e border-slate-200 bg-white transition-transform duration-200 md:sticky md:top-0 md:h-screen md:translate-x-0 dark:border-slate-800 dark:bg-slate-950">
            ...
            @php
                ...
                // Content group: show the full menu when the current website has
                // content access (or already-generated content to publish);
                // otherwise collapse to a single "Get started" entry. Guarded so
                // it can never 500 the whole app (e.g. before content migrations).
                $contentNavFull = false;
                if (\Illuminate\Support\Facades\Route::has('content.index') && $authUser) {
                    try {
                        $curWeb = $currentWebsiteId !== ''
                            ? $authUser->accessibleWebsitesQuery()->whereKey($currentWebsiteId)->first()
                            : null;
                        $curWeb ??= $authUser->accessibleWebsitesQuery()->first();
                        if ($curWeb) {
                            $contentNavFull = app(\App\Services\Content\ContentEntitlements::class)->hasContentAccessFor($authUser, $curWeb)
                                || \App\Models\ContentPlan::where('website_id', $curWeb->id)->whereHas('topics')->exists();
                        }
                    } catch (\Throwable) {
                        $contentNavFull = false;
                    }
                ...
                $navGroups = [
                    ...
                    [
                        ...
                        // With the SEO UI off, content IS the product: no label
                        // -> the items render flat at the top of the sidebar
                        // instead of collapsed behind a "Content" flyout.
                        'label' => config('features.seo_platform_ui') ? 'Content' : null,
                        'items' => ! (\Illuminate\Support\Facades\Route::has('content.index') && \Illuminate\Support\Facades\Route::has('content.get-started')) ? []
                            : ($contentNavFull ? [
                            ['route' => 'content.index', 'feature' => 'content', 'label' => __('Content Calendar'), 'icon' => 'M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5'],
                            ['route' => 'content.research', 'feature' => 'content', 'label' => __('Research'), 'badge' => __('New'), 'icon' => 'M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z'],
                            ['route' => 'content.settings', 'feature' => 'content', 'label' => config('features.seo_platform_ui') ? __('Settings') : __('Content Settings'), 'icon' => 'M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.324.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 011.37.49l1.296 2.247a1.125 1.125 0 01-.26 1.431l-1.003.827c-.293.24-.438.613-.431.992a6.759 6.759 0 010 .255c-.007.378.138.75.43.99l1.005.828c.424.35.534.954.26 1.43l-1.298 2.247a1.125 1.125 0 01-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.57 6.57 0 01-.22.128c-.331.183-.581.495-.644.869l-.213 1.28c-.09.543-.56.941-1.11.941h-2.594c-.55 0-1.02-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 01-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 01-1.369-.49l-1.297-2.247a1.125 1.125 0 01.26-1.431l1.004-.827c.292-.24.437-.613.43-.992a6.932 6.932 0 010-.255c.007-.378-.138-.75-.43-.99l-1.004-.828a1.125 1.125 0 01-.26-1.43l1.297-2.247a1.125 1.125 0 011.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.087.22-.128.332-.183.582-.495.644-.869l.214-1.281z'],
                            ...
                            ['route' => 'content.site-health', 'feature' => 'content', 'label' => __('Site Health'), 'badge' => __('New'), 'icon' => 'M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z'],
                            ['route' => 'content.ai-visibility', 'feature' => 'content', 'label' => __('AI Visibility'), 'badge' => __('New'), 'icon' => 'M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 00-2.456 2.456z'],
                            ...
                            // Auto-share is its own destination, not a card buried in
                            // Integrations. Hidden entirely unless a provider's OAuth
                            // app is configured — same gate the page itself uses, so
                            // the link can never lead to an empty screen.
                            ...(\App\Services\Content\Social\SocialPoster::anyProviderConfigured() ? [
                                ['route' => 'content.social', 'feature' => 'content', 'label' => __('Auto-share'), 'badge' => __('New'), 'icon' => 'M7.217 10.907a2.25 2.25 0 100 2.186m0-2.186c.18.324.283.696.283 1.093s-.103.77-.283 1.093m0-2.186l9.566-5.314m-9.566 7.5l9.566 5.314m0 0a2.25 2.25 0 103.935 2.186 2.25 2.25 0 00-3.935-2.186zm0-12.814a2.25 2.25 0 103.933-2.185 2.25 2.25 0 00-3.933 2.185z'],
                            ] : []),
                        ] : [
                    ...
                    [
                        ...
                        'items' => [
                            ...
                            // Team hidden in content-only mode (owner request
                            // 2026-08-08) — the page itself stays reachable.
                            ...(config('features.seo_platform_ui') ? [
                            ...
                            // Referral program — per-user, survives the SEO
                            // kill-switch. Heroicon gift outline.
                            ['route' => 'referrals.index', 'feature' => null, 'label' => __('Refer & earn'), 'icon' => 'M21 11.25v8.25a1.5 1.5 0 01-1.5 1.5H5.25a1.5 1.5 0 01-1.5-1.5v-8.25M12 4.875A2.625 2.625 0 109.375 7.5H12m0-2.625V7.5m0-2.625A2.625 2.625 0 1114.625 7.5H12m0 0V21m-8.625-9.75h18c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125h-18c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z'],
