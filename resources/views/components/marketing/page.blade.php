            ...
            <nav aria-label="Primary" class="hidden items-center gap-7 text-sm text-slate-600 md:flex">
                ...
                <div class="relative" x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false">
                    <button type="button" @click="open = !open" :aria-expanded="open.toString()"
                        class="inline-flex items-center gap-1 transition hover:text-slate-900 {{ $active === 'pricing' ? 'text-slate-900' : '' }}">
                        {{ __('Pricing') }}
                        <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>
                    </button>
                    <div x-show="open" x-cloak x-transition.opacity
                         class="absolute start-0 top-full z-20 w-64 pt-3">
                        <div class="overflow-hidden rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl">
                            <a href="{{ route('pricing') }}" class="block rounded-lg px-3 py-2.5 transition hover:bg-slate-50">
                                <span class="block text-sm font-semibold text-slate-900">{{ __('SEO Platform') }}</span>
                                <span class="block text-xs text-slate-500">{{ __('Audits, rank tracking, reports') }}</span>
                            </a>
                            <a href="{{ route('content.pricing') }}" class="block rounded-lg px-3 py-2.5 transition hover:bg-slate-50">
                                <span class="block text-sm font-semibold text-slate-900">{{ __('Content AI Autopilot') }}</span>
                                <span class="block text-xs text-slate-500">{{ __('Articles written & published') }}</span>
                            </a>
                        </div>
                    </div>
                </div>
