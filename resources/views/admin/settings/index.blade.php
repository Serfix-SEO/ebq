    ...
    <div class="mx-auto max-w-2xl">
        <div class="mb-6">
            ...
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                Platform defaults: AI provider & models, rank-tracker re-check interval, and Keywords Everywhere usage.
            </p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.settings.update') }}" class="space-y-6">
            ...
            <section class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <h2 class="text-sm font-semibold text-slate-900 dark:text-slate-100">AI provider</h2>
                <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                    Which provider powers every AI feature (brief, writer, strategy tools, chat, custom-prompt classifier), and the default model per provider. Per-call overrides still win when a service pins a model. Image alt-text always uses Mistral's vision model regardless of this choice.
                </p>

                <label for="llm_provider" class="mt-4 block text-xs font-semibold text-slate-700 dark:text-slate-300">Active provider</label>
                <select id="llm_provider" name="llm_provider"
                    ...
                    @foreach ($llmProviders as $value => $label)
                        <option value="{{ $value }}" @selected(old('llm_provider', $llmProvider) === $value)>{{ $label }}</option>
                    @endforeach
                </select>

                <label for="model" class="mt-4 block text-xs font-semibold text-slate-700 dark:text-slate-300">Mistral default model</label>
                <select id="model" name="model"
                    ...
                    @foreach ($mistralModels as $m)
                        <option value="{{ $m['id'] }}" @selected(old('model', $currentMistralModel) === $m['id'])>
                            {{ $m['label'] ?? $m['id'] }}
                        </option>
                    @endforeach
                </select>

                <label for="deepseek_model" class="mt-4 block text-xs font-semibold text-slate-700 dark:text-slate-300">DeepSeek default model</label>
                <select id="deepseek_model" name="deepseek_model"
                    class="mt-1 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800 focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100">
                    @foreach ($deepseekModels as $m)
                        <option value="{{ $m['id'] }}" @selected(old('deepseek_model', $currentDeepseekModel) === $m['id'])>
                            {{ $m['label'] ?? $m['id'] }}
                        </option>
                    @endforeach
                </select>
                ...
                <p class="mt-2 text-[11px] text-slate-500 dark:text-slate-400">
                    Currently active: <code class="rounded bg-slate-100 px-1.5 py-px font-mono text-[11px] dark:bg-slate-800 dark:text-slate-200">{{ $llmProviders[$llmProvider] ?? $llmProvider }} · {{ $llmProvider === 'deepseek' ? $currentDeepseekModel : $currentMistralModel }}</code>
                </p>
            </section>
            ...
            <section class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                ...
                <div class="mt-2 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    @foreach ([
                        ...
                        'content_catalog_page_budget' => ['Product catalog: pages per scan', 'content_catalog_page_budget', 100, 50000],
                        'content_catalog_llm_extract_cap' => ['Product catalog: LLM-extract cap / run', 'content_catalog_llm_extract_cap', 0, 2000],
                        'content_catalog_firecrawl_daily_budget' => ['Product catalog: Firecrawl renders / day', 'content_catalog_firecrawl_daily_budget', 0, 5000],
        ...
        <form method="POST" action="{{ route('admin.settings.refresh-models') }}" class="mt-4 flex justify-end">
            ...
            <button class="text-xs font-semibold text-orange-600 hover:underline dark:text-orange-400">
                Refresh AI model lists from providers
            </button>
        </form>
    </div>
</x-layouts.app>
