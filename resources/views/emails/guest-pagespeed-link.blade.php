...
<head>
    ...
    <title>{{ __('Your free PageSpeed report') }}</title>
</head>
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f1f5f9; padding:24px 12px;">
        <tr>
            <td align="center">
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px; background:#ffffff; border:1px solid #e2e8f0; border-radius:14px; overflow:hidden;">
                    <tr>
                        <td style="padding:28px 32px 8px;">
                            <p style="margin:0 0 4px; font-size:11px; font-weight:700; letter-spacing:0.12em; text-transform:uppercase; color:#F26419;">{{ __('Free PageSpeed report') }}</p>
                            <h1 style="margin:0; font-size:22px; line-height:1.3; color:#0f172a;">{{ __('Your PageSpeed report for :host is ready', ['host' => $host]) }}</h1>
                            <p style="margin:14px 0 0; font-size:14px; line-height:1.6; color:#475569;">
                                {{ __('We measured') }} <strong style="color:#0f172a;">{{ $report->url }}</strong> {{ __('on mobile and desktop —') }}
                                {{ __('Lighthouse scores, Core Web Vitals, and the exact resources to fix.') }}
                            </p>
                        </td>
                    </tr>
                    <tr>
                            ...
                            <table role="presentation" cellpadding="0" cellspacing="0">
                                <tr>
                                    <td style="border-radius:10px; background:#F26419;">
                                        <a href="{{ $resultsUrl }}" style="display:inline-block; padding:13px 26px; font-size:14px; font-weight:600; color:#ffffff; text-decoration:none; border-radius:10px;">{{ __('View your PageSpeed report →') }}</a>
                                    </td>
                                </tr>
                            </table>
                                ...
                                <tr>
                                    <td style="padding:18px 20px;">
                                        ...
                                        <p style="margin:8px 0 14px; font-size:13px; line-height:1.6; color:#475569;">
                                            {{ __('Create a free account to monitor Core Web Vitals continuously, run full SEO audits, and connect Search Console + Analytics for live keyword and traffic data. No credit card.') }}
                                        </p>
                ...
                <p style="margin:16px 0 0; font-size:11px; color:#94a3b8;">{{ __('You’re receiving this because you requested a free PageSpeed test at Serfix.') }}</p>
            </td>
        </tr>
    </table>
</body>
