{{-- Shared "analyze any domain" input — used by the Explorer entry page AND
     the top of every report page (explore-the-next-domain flow). Submits to
     POST /analyze so the per-plan Site Explorer lookup limits apply (429 →
     inline message); success redirects to the new report.
     Params: $placeholder (optional). --}}
<form id="se-form" data-action="{{ route('analyze.store') }}" novalidate>
    @csrf
    <div class="flex flex-col gap-2 rounded-2xl bg-white p-2 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800 sm:flex-row sm:items-center">
        <div class="flex min-w-0 flex-1 items-center gap-3 px-3 py-2">
            ...
            <input id="se-url" name="url" type="text" inputmode="url" autocomplete="url" required
    ...
    <p id="se-error" role="alert" class="mx-auto mt-3 hidden max-w-md rounded-lg bg-rose-50 px-3 py-2 text-center text-[13px] font-medium text-rose-700 ring-1 ring-rose-100 dark:bg-rose-500/10 dark:text-rose-400 dark:ring-rose-500/20"></p>
</form>
