...
@php
    ...
    $compLabel = $competition === null ? '—'
        : ($competition < 0.34 ? __('Low') : ($competition < 0.67 ? __('Medium') : __('High')));
...
<section>
    <div class="grid gap-4 sm:grid-cols-3">
        ...
        <div class="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white p-6 text-center shadow-sm ring-1 {{ $volume !== null ? 'ring-orange-200' : 'ring-slate-200' }}">
            <p class="text-[11px] font-semibold uppercase tracking-wider text-slate-400">{{ __('Monthly searches') }}</p>
            ...
            <p class="mt-2 text-xs font-medium text-slate-500">{{ $volume !== null ? __('avg. searches / month') : __('No volume data for this keyword') }}</p>
        </div>

        {{-- CPC + competition --}}
        <div class="sm:col-span-2 grid grid-cols-2 gap-4">
            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <p class="text-[11px] font-semibold uppercase tracking-wider text-slate-400">{{ __('Cost per click') }}</p>
                <p class="mt-1 text-2xl font-bold tabular-nums text-slate-900">{{ $cpc !== null ? $currency.' '.number_format((float) $cpc, 2) : '—' }}</p>
                <p class="mt-1 text-xs text-slate-500">{{ __('Google Ads top-of-page bid') }}</p>
            </div>
            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <p class="text-[11px] font-semibold uppercase tracking-wider text-slate-400">{{ __('Competition') }}</p>
                <p class="mt-1 text-2xl font-bold {{ $compTone }}">{{ $compLabel }}</p>
                <p class="mt-1 text-xs text-slate-500">{{ $competition !== null ? __('Index :value', ['value' => number_format((float) $competition, 2)]) : __('Advertiser density') }}</p>
            </div>
            <div class="col-span-2 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
                <p class="px-1 text-[11px] font-semibold uppercase tracking-wider text-slate-400">{{ __('12-month trend') }}</p>
                @if (count($series))
                ...
                @else
                    <p class="px-1 py-4 text-center text-xs text-slate-400">{{ __('No trend data available.') }}</p>
                @endif
            </div>
        </div>
    </div>
    ...
    <p class="mt-5 text-[11px] leading-relaxed text-slate-400">
        {{ __('Volume, CPC and competition from Keywords Everywhere (Google Keyword Planner data) for') }}
        <strong class="text-slate-500">{{ $result['keyword'] ?? '' }}</strong> · {{ \App\Support\KeywordsEverywhereCountries::label($result['country'] ?? 'global') }}@if (! empty($result['fetched_at'])) · {{ __('updated :time', ['time' => \Carbon\Carbon::parse($result['fetched_at'])->diffForHumans()]) }}@endif.
    </p>
</section>
