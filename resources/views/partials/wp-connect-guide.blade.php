{{--
    ...
    The screens are drawn as inline SVG rather than screenshots: they stay sharp
    at any size, follow the light/dark theme, ship no extra asset requests, and
    let the exact field being talked about be highlighted. They are stylised —
    a customer's WordPress may be a different version or language, so every
    instruction also names the menu path in words, which is what people
    actually search for.
--}}
@php
    // Illustration palette, resolved once so light/dark stays consistent.
...
<details class="mt-4 rounded-xl border border-slate-200 bg-slate-50/60 dark:border-slate-700 dark:bg-slate-800/40" open>
    ...
    <div class="space-y-5 border-t border-slate-200 px-4 py-4 dark:border-slate-700">
        ...
        {{-- Step 2 + illustration of the profile screen --}}
        <div class="flex gap-3">
            ...
            <div class="min-w-0 flex-1">
                ...
                <p class="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                    {{ __('It sits near the bottom of your own profile page, under “Account Management”.') }}
                </p>

                <svg viewBox="0 0 420 190" class="mt-3 w-full max-w-lg rounded-lg border border-slate-200 dark:border-slate-700" role="img"
                     aria-label="{{ __('WordPress profile page with the Application Passwords section near the bottom') }}">
                    ...
                    <rect x="98" y="92" width="304" height="80" rx="6" class="fill-orange-50 dark:fill-orange-950/40" stroke-width="1.5"
                          style="stroke:#F26419"/>
        ...
        <div class="flex gap-3">
            ...
            <div class="min-w-0">
                <p class="text-sm font-semibold text-slate-800 dark:text-slate-100">{{ __('Name it “Serfix” and click Add New Application Password') }}</p>
                <p class="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                    {{ __('The name is only a label so you can recognise (and revoke) it later.') }}
                </p>
            </div>
        </div>

        {{-- Step 4 + illustration of the reveal --}}
        <div class="flex gap-3">
            <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-orange-600 text-xs font-bold text-white">4</span>
            <div class="min-w-0 flex-1">
                <p class="text-sm font-semibold text-slate-800 dark:text-slate-100">{{ __('Copy the password WordPress shows you') }}</p>
                <p class="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                    {{ __('It looks like four-character groups. WordPress shows it ONCE — if you close the page before copying, delete it and create another.') }}
                </p>

                <svg viewBox="0 0 420 96" class="mt-3 w-full max-w-lg rounded-lg border border-slate-200 dark:border-slate-700" role="img"
                     aria-label="{{ __('WordPress showing the newly generated application password once') }}">
                    ...
                    <text x="28" y="36" style="font-size:8px;font-weight:700" class="fill-emerald-800 dark:fill-emerald-300">
                        {{ __('Your new password for Serfix is:') }}
                    </text>
        ...
        {{-- Step 5 --}}
        <div class="flex gap-3">
            <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-orange-600 text-xs font-bold text-white">5</span>
            <div class="min-w-0">
                <p class="text-sm font-semibold text-slate-800 dark:text-slate-100">{{ __('Paste it below, with your site address and WordPress username') }}</p>
                <p class="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                    {{ __('Spaces in the password are fine — paste it exactly as shown. Use your login username, not your display name or email.') }}
                </p>
            </div>
        </div>

        {{-- Troubleshooting: this is where connections actually fail. --}}
        <div class="rounded-lg border border-amber-200 bg-amber-50/70 px-4 py-3 dark:border-amber-900/60 dark:bg-amber-950/30">
            <p class="text-xs font-bold text-amber-900 dark:text-amber-200">{{ __('Don’t see “Application Passwords”?') }}</p>
