    ...
    <table style="width: 100%; margin-top: 14px; border-collapse: separate; border-spacing: 12px 0;">
        <tr>
            <td style="width: 50%; vertical-align: top; padding: 12px 14px; border-radius: 10px; border: 1px solid #e2e8f0; background: #ffffff;">
                <p style="margin: 0; font-size: 9px; font-weight: 800; letter-spacing: 0.12em; text-transform: uppercase; color: #64748b;">{{ __('SERP sample (organic listing)') }}</p>
                <p style="margin: 6px 0 0; font-size: 10px; line-height: 1.45; color: #64748b;">{{ __('From live search results for the benchmark keyword.') }}</p>
                @if ($hasSerpListing)
                ...
                @else
                    <p style="margin: 12px 0 0; font-size: 12px; color: #64748b;">{{ __('No organic row matched your site’s domain in this snapshot.') }}</p>
                @endif
            </td>
            <td style="width: 50%; vertical-align: top; padding: 12px 14px; border-radius: 10px; border: 1px solid #FBD1BB; background: #FEF0E7;">
                <p style="margin: 0; font-size: 9px; font-weight: 800; letter-spacing: 0.12em; text-transform: uppercase; color: #9F3D09;">{{ __('Your page (audit snapshot)') }}</p>
                <p style="margin: 6px 0 0; font-size: 10px; line-height: 1.45; color: #475569;">{{ __('Title and meta description from fetched HTML.') }}</p>
                @if ($hasAudited)
                    ...
                    @else
                        <p style="margin: 8px 0 0; font-size: 13px; font-style: italic; color: #64748b;">{{ __('No meta description in HTML.') }}</p>
                    @endif
                ...
                @else
                    <p style="margin: 12px 0 0; font-size: 12px; color: #64748b;">{{ __('No on-page snapshot stored (re-run the audit).') }}</p>
                @endif
            </td>
        </tr>
    </table>
